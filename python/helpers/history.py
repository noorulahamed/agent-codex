...
class Topic(Record):
    ...
    async def summarize_messages(self, messages: list[Message]):
        """
        Summarize a list of messages using the utility LLM.
        
        Args:
            messages: List of Message objects to summarize
            
        Returns:
            str: Summarized text generated by the utility LLM
            
        Note:
            Currently vision bytes are sent to utility LLM via output_text().
            For efficiency, consider extracting and summarizing vision content 
            separately before sending to LLM in future iterations.
        """
